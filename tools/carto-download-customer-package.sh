...
# Check if exist a stable release
...
then
  ...
  SELFHOSTED_VERSION_LATEST_RC="$(echo "${CUSTOMER_PACKAGE_FILE_LATEST}" | grep -Eo "${CLIENT_ID}-[0-9]+-[0-9]+-[0-9]+-rc-[0-9]+")"
  SELFHOSTED_VERSION_LATEST_RC="${SELFHOSTED_VERSION_LATEST_RC/#${CLIENT_ID}-}"
  CUSTOMER_PACKAGE_DOWNLOAD_URL="gs://${CLIENT_STORAGE_BUCKET}/${CUSTOMER_PACKAGE_FOLDER}/${CUSTOMER_PACKAGE_NAME_PREFIX}-${CLIENT_ID}-${SELFHOSTED_VERSION_LATEST_RC}.zip"
fi
